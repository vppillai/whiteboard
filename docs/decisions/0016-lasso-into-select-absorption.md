...
5. **Group drag** — pointer-down on any selected object (single or part of a multi) initiates a drag that moves the whole selection together. `commitDrag` walks the selection and emits one op per displaced item (a single `move` op for all displaced strokes; one `transform-image` per displaced image; one `transform-text` per displaced text). Undo restores all in one step because the ops queue commits in batch. **Post-acceptance, the per-item ops were coalesced into a single composite `transform-many` op — one undo step structurally, and one transaction / wire message per peer under future sync (see the rationale comment in `ops.ts`).**
6. **Group delete** — `Delete` / `Backspace` walks the selection and dispatches the per-kind delete op for each item. **Post-acceptance, this too was coalesced: `deleteSelected` emits a single composite `delete-many` op (stroke + image + text + shape id lists), and the whiteboard-native paste path emits a matching composite `create-many`, so group delete and group paste each undo in one step.**
7. **PNG clipboard for multi-selection.** `Cmd/Ctrl+C` / `Cmd/Ctrl+X` writes a transparent-background PNG of the selection's bbox to the system clipboard (via the existing `writePngBlobToClipboard` path; same renderer as the export pipeline, `transparentBg=true`). The single-image-selection fast path still writes raw image bytes (preserves the original encoding for round-trip fidelity). **Post-acceptance, the multi-selection clipboard evolved into a dual-slot bundle (`image/png` + `text/html` carrying `data-whiteboard-v1`) so strokes + texts round-trip as live vectors when pasted back inside the whiteboard, while still landing as PNG in external apps. See [ADR 0017](0017-whiteboard-native-clipboard-format.md).**
...
- **0017 Whiteboard-native clipboard format** — the dual-slot clipboard bundle that supersedes the PNG-only multi-selection clipboard called out in item 7 above.
