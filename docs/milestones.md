...
**v1.5.1 shipped 2026-07-17.** Single-user, offline-first whiteboard with strokes, image paste, text, laser, unified Select multi-object workflows, whiteboard-native clipboard round-trip, Shape tool support (rect / ellipse / line / arrow), plus post-v1.4.0 hardening: factory-reset URL cleanup, tool-menu TDZ fix, op-pipeline funneling for deletes, text measurement + pen halo pre-render caches, server CSP/security headers + path-traversal guard, and the standard OSS community-health files; a v1.4.5 cleanup & frontend pass — keyboard focus rings, a documented z-index scale, export error handling, and internal dedup; a v1.4.6 review bug-fix pass — eleven verified defects (live-DPR tracking, IndexedDB open-retry, distraction-free chrome, mid-drag text commit, full reset-to-defaults, rotated-paste anchor, editable-target key guards) plus an SVG-export injection fix; the v1.5.0 review-roadmap completion — composite one-step group undo, a Playwright e2e smoke suite gating CI, `ObjectStore<T>` / `ObjectBehavior<T>` architecture unification per ADR 0014, and CI/deploy supply-chain pinning + container confinement; and a v1.5.1 UX bug-fix pass — twenty verified defects from a six-dimension frontend review (resize-blanked canvas, DPR tracking via matchMedia, wheel deltaMode normalization, single-owner pointer routing, line/arrow endpoint handles, stale text-measurement cache, destructive-toast focus/orphan safety, pagehide edit flush, persistence-failure toast). Live collaboration remains deferred per [ADR 0012](decisions/0012-sharing-deferred.md). PWA install + deployment polish are post-v1 work tracked below.
...
| **v1.5.0** | Review roadmap completion — composite group undo, Playwright e2e suite + CI job, `ObjectStore<T>` + `ObjectBehavior<T>` unification (ADR 0014 adopted), supply-chain pinning + container confinement | ✅ *(2026-06-10; tag `v1.5.0`)* |
| **v1.5.1** | UX bug-fix pass — 20 verified defects across input, select, text, camera/render, chrome, persistence; new chokepoints: `editable.ts` guards, `setupCanvas(onResize)`, `Tool.onPointerLeave`, `enabledHandles`, invalidate-inside-`resizeToFit` | ✅ *(2026-07-17; tag `v1.5.1`)* |
