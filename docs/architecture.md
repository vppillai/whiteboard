...
| `storage.ts`    | M0 / v1.1 / v1.2 / v1.3 ✅ | Local persistence via IndexedDB. Schema history: v1 (strokes) → v2 (+ images + images-blob, M2.2) → v3 (+ texts, v1.2) → v4 (corrective re-upgrade; recovers users whose v3 upgrade missed the `texts` store). Existing data preserved across every bump via idempotent `if (!contains)` create guards. Migration constraint: those guards are safe ONLY for create-new-store paths — mutations to an existing store need an explicit `event.oldVersion < N` guard. |
...
| `selectionclipboard.ts` | v1.3 ✅ | Copy / cut / paste-back subsystem for the Select tool's current selection. Path selection: single-image selection → raw bytes (best fidelity, preserves original format); selection with no images → whiteboard-native bundle + rendered PNG (vector round-trip + external paste); selection with images → PNG-only (image-bytes round-trip deferred). Paste-back walks the bundle, freshly id's each item via `ids.ts`, translates by `(cursor - origin)`, dispatches `create` / `create-text` ops, then auto-switches to Select and pre-selects the pasted group. Parallel to `imagepaste.ts` with the same Context-injected dependency pattern; testable in isolation. |
...
| **Whiteboard-native clipboard round-trip** | ✅ Complete | v1.3; `Cmd/Ctrl+C` / `Cmd/Ctrl+X` writes a dual-slot clipboard payload (`image/png` for external apps + `text/html` carrying a `data-whiteboard-v1` data attribute with the strokes + texts bundle). Pasting back inside the whiteboard reconstitutes the objects as live vectors; external paste lands as PNG. Selections containing images fall back to PNG-only (image-bytes round-trip deferred). DoS-capped at 5000 strokes / 5000 texts / 50000 samples per stroke. See [ADR 0017](decisions/0017-whiteboard-native-clipboard-format.md). |
