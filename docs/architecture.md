...
Vanilla TypeScript SPA built with Vite. Renders to `<canvas>` directly — no React, no Solid, no UI framework on the drawing surface.
...
| `tools/select.ts`| v1.1 / v1.2 / v1.3 / v1.4 ✅ | Select tool (`V` or `S`) — universal selection for any board object (image, text, shape, stroke), single or multi. `selected: Selection[]` where `Selection = { kind, id }` spans all object kinds. Single-selection drives per-kind handles (images/text/shapes get resize + rotation affordances; strokes are move-only). Multi-selection supports marquee + `Shift+click` toggle + group move/delete + `Cmd/Ctrl+A` select-all. `Cmd/Ctrl+C` / `Cmd/Ctrl+X` routes through the selection clipboard pipeline (native whiteboard bundle + PNG fallback semantics). Per-kind capability dispatch (live-object resolution, view, drag commits, multi-drag, delete slots) routes through the `ObjectBehavior<T>` registry in `tools/select/behaviors.ts`; the tool file keeps the gesture state machine plus the intentionally-kind-specific sites (hit-test ordering, marquee semantics, resize math, selection paint). Three self-contained blocks live in sibling modules: the behavior vtable in `tools/select/behaviors.ts`, pure handle geometry in `tools/select/handles.ts`, and the contextual-menu DOM builders in `tools/select/menu.ts`. See [ADR 0014](decisions/0014-select-tool-selection-union.md) and [ADR 0016](decisions/0016-lasso-into-select-absorption.md). |
| `tools/select/behaviors.ts` | v1.4 ✅ | Per-kind `ObjectBehavior<T>` vtable for the Select tool — the polymorphism step [ADR 0014](decisions/0014-select-tool-selection-union.md)'s migration trigger promised, fired at four object kinds. A registry keyed by selection kind exposing `resolve` / `viewParts` / `beginMultiDrag` (per-item closure handle with tick/commit) / `commitSingleDrag` / `collectDeleteId`. The rect-transform kinds (image / text / shape) share generic helpers; the stroke behavior keeps its samples-translation model (dx/dy `move` semantics) instead of a forced transform shape. |
| `tools/select/handles.ts` | v1.4 ✅ | Pure handle geometry for the Select tool — `handlePositions` / `rotationHandlePos` / `anchorBoardFor` / `cursorFor` / `handleAt` / `isOverRotationHandle`, plus the `HandleId` type. Every function is a function of (transform, rotation, scale) only — no selection state, no DOM. |
| `tools/select/menu.ts` | v1.4 ✅ | Contextual-menu DOM builders for the Select tool — the shape menu (Color / Stroke width / Fill / Fill opacity) and text menu (Color / Font / Size / B / I / U). Pure DOM construction over a live object; tool internals (drag commit, persistence, op push) injected via explicit deps interfaces. `select.ts`'s `renderContextualMenu` stays as the dispatcher. |
| `tools/laser.ts` | v1.2 ✅  | Laser pointer (`P`) — ephemeral fading polyline trail. Each pen-down draws a disjoint stroke (per-sample `continueFromPrev`); trails fade over ~2.2 s via a self-driving RAF loop. `lineCap = butt` + per-span average-age alpha. No persistence / op-pipeline / export integration. `laserColor` setting separate from pen color (default `#ef4444`). |
...
| `ops.ts`        | M1 / v1.1 / v1.2 / v1.3 / v1.4 ✅ | Op-based undo. Stroke kinds: `create` / `delete` / `move` / `eraseStamps` (ADRs 0006 + 0009). The `move` op is emitted by the Select tool — for a single click-selected stroke (v1.2 post-release) and for every displaced stroke in a multi-selection group drag (v1.3 post-Lasso-absorption). Image kinds: `paste-image` / `delete-image` / `transform-image` / `rotate-image` (v1.1). Text kinds: `create-text` / `delete-text` / `transform-text` / `edit-text` / `rotate-text` (v1.2). Shape kinds: `create-shape` / `delete-shape` / `transform-shape` / `rotate-shape` (v1.4), plus cross-kind composite ops (`transform-many`, `delete-many`) used for unified multi-object moves/deletes. `edit-text` payload carries `{ content, font, color, wrapWidth }` so undo restores wrap-width alongside font changes. A generic `flipDeletedOn` helper unifies the per-kind soft-delete undo paths. |
| `stroke.ts`     | M0 ✅    | Stroke geometry via `perfect-freehand`; pressure curve; `erasedStamps` helpers. |
| `render.ts`     | M1 ✅    | Three-layer canvas: `committed` (grid + composited strokes) + `strokes` (offscreen, destination-out target) + `live` (in-flight + cursor). ADR 0009 § *Renderer*. Within-committed pass order: grid → images → texts → shapes → composited strokes ([ADR 0018](decisions/0018-shape-tool.md) § 4). New object-kind renderers insert before the strokes composite. |
...
| `storage.ts`    | M0 / v1.1 / v1.2 / v1.3 / v1.4 ✅ | Local persistence via IndexedDB. Schema history: v1 (strokes) → v2 (+ images + images-blob, M2.2) → v3 (+ texts, v1.2) → v4 (corrective re-upgrade; recovers users whose v3 upgrade missed the `texts` store) → v5 (+ shapes, v1.4). Existing data preserved across every bump via idempotent `if (!contains)` create guards. Migration constraint: those guards are safe ONLY for create-new-store paths — mutations to an existing store need an explicit `event.oldVersion < N` guard. |
| `objectstore.ts` | v1.4 ✅ | Generic `ObjectStore<T>` interface — the single store contract (load / upsert / delete / clear + `onRemoteChange` no-op stub) shared by all four object stores, so a future Y.Doc-backed sync implementation (ADR 0012) is ONE adapter, not four. |
| `imagestore.ts` | v1.1 ✅  | `ImageStore` seam — extends `ObjectStore<ImageObject>` with a blob side channel: `insert(image, blob)` creation path, `loadBlob`, blob-aware `delete(id, blobRef)`, and a `load` that also reports compacted blobRefs. Local IDB-backed impl. Image binaries deferred to M5.1 sync per ADR 0012. |
...
| `geom.ts`       | v1.4 ✅  | Shared rotation-aware geometry primitives — `ROTATION_EPSILON = 1e-9` (float-drift-safe fast-paths), `rotateAroundPoint`, generic `rotatedRectAABB` (rotated-rect corner math behind `imageAABB` / `textAABB` / `shapeAABB`), and the `ViewBBox` interface the render passes cull against. DOM-free so tests and export serializers can consume it. |
| `selectionoutline.ts` | v1.4 ✅ | Shared dashed multi-selection outline painter used by the image / text / shape render passes — one implementation of the ~2px-at-any-zoom accent outline (was three copies). |
| `imagegeom.ts`  | v1.1 ✅  | Image-specific rotation geometry: `imageCenter`, `rectCorners`, `imageAABB`, `pointInImage`. Delegates the shared primitives (`rotateAroundPoint`, `ROTATION_EPSILON`, rotated-rect AABB) to `geom.ts`. |
| `renderimages.ts`| v1.1 ✅ | Per-frame image render pass — viewport cull via rotation-aware AABB, rotation transform, dashed multi-selection outline. Single `renderImages({ images, layer, camera, viewBBox, isMultiSelected })` entry point. The `isMultiSelected` predicate is backed by a per-frame `Set<string>` (constructed once in `main.ts` from `selected[]`) so the lookup is O(1) per image and total render-pass cost is O(N+M) instead of the pre-rename O(N×M). |
| `textstore.ts`   | v1.2 ✅  | `TextStore` seam — implements `ObjectStore<TextObject>` verbatim. Single-store concrete impl (no companion blob — text payload is inline and ships with the record in v1.2). Only image binaries are deferred to M5.1 sync per ADR 0012; text payloads are part of the shipped data model. |
...
| `tools/shape.ts` | v1.4 ✅  | Shape tool (`R` / `O` / `A` / `L`) — rectangle / ellipse / arrow / line creation with shift-constrained geometry and sticky style defaults. |
| `shapestore.ts`  | v1.4 ✅  | `ShapeStore` seam — implements `ObjectStore<ShapeObject>` verbatim (load / upsert / delete / clear flows). |
| `rendershapes.ts`| v1.4 ✅  | Per-frame shape render pass with fill/stroke drawing, rotation-aware culling, and selection outline support. |
...
| `ids.ts`        | v1.3 ✅  | Object id generation. `makeStrokeId` (`s_<uuid>`) / `makeTextId` (`t_<uuid>`) centralized so the prefix convention stays consistent across every creator (pen tool, text tool, clipboard paste-back). Uses `crypto.randomUUID()` with a `<prefix>_<base36-time>_<base36-rand>` fallback for non-secure-context / older browsers. |
| `rendertexts.ts` | v1.2 ✅  | Per-frame text render pass mirroring `renderimages.ts`. Skips the in-edit text id so the DOM overlay doesn't double up with the canvas render. Rotation-aware AABB cull + dashed outline for Cmd+A multi-select halos via the per-frame `isMultiSelected` Set. Uses `measureText.lines / lineWidths` so wrap-width text renders as wrapped, not just hard-newline-split. |
| `selectionclipboard.ts` | v1.3 / v1.4 ✅ | Copy / cut / paste-back subsystem for the Select tool's current selection. Path selection: single-image selection → raw bytes (best fidelity, preserves original format); selection with no images → whiteboard-native bundle + rendered PNG (vector round-trip + external paste); selection with images → PNG-only (image-bytes round-trip deferred). Paste-back walks the bundle, freshly id's each item via `ids.ts`, translates by `(cursor - origin)`, dispatches `create` / `create-text` / `create-shape` ops, then auto-switches to Select and pre-selects the pasted group. Parallel to `imagepaste.ts` with the same Context-injected dependency pattern; testable in isolation. |
...
- `ImageObject`, `TextObject`, `ShapeObject`, `TextFontFamily` types.
...
`apps/web/src/storage.ts` wraps IndexedDB. One database (`whiteboard-local`) with stores for `strokes`, `images`, `images-blob`, `texts`, and `shapes`. Writes happen at commit boundaries (`pointerup` and drag commit paths), so a power-loss event at most loses in-flight edits. Reads on app boot hydrate the committed canvas.

Wrapped behind the generic `ObjectStore<T>` interface seam (M2.1 as `StrokeStore`, unified across all four stores in `apps/web/src/objectstore.ts`) so the persistence backend can be swapped without touching the orchestrator — preserving the option to attach a future Y.Doc-backed implementation when sharing returns.
...
| CI                                | ✅ Complete    | GitHub Actions: lint + typecheck + tests + Docker smoke. |
...
| Local persistence (M0+)           | ✅ Complete    | IndexedDB; auto-save on commit boundaries for strokes/images/text/shapes; hydrate on load. `erasedStamps` persisted alongside strokes. |
...
| **Brushes (pen / marker / pencil / highlighter / brush)** | ✅ Complete | M1; 1–5 keys (or `Shift+P` for pen default) + right-click menu BRUSH section. |
...
| **`Shift+P` = Draw + Pen preset** | ✅ Complete    | M1; one-keystroke "go to my default drawing setup."     |
...
| **Select tool — universal selection** | ✅ Complete    | M1 (originally as `Lasso`) / v1.1 (Select tool for images) / v1.2 (Select extension to texts + click-to-select strokes) / v1.3 post-release (Lasso absorbed into Select; `Selection[]` array; marquee drag; `Shift+click` toggle; `Cmd/Ctrl+A` selects everything across kinds; group move + delete; dual-slot clipboard for multi-selection). Drag state unified into a discriminated union (`ActiveDrag = { kind: 'single' \| 'multi' \| 'marquee'; state }`); mutual exclusion enforced by the type system. `V` or `S` activates; `Delete`/`Backspace` deletes. See [ADR 0014](decisions/0014-select-tool-selection-union.md) + [ADR 0016](decisions/0016-lasso-into-select-absorption.md). |
| **Whiteboard-native clipboard round-trip** | ✅ Complete | v1.3 / v1.4; `Cmd/Ctrl+C` / `Cmd/Ctrl+X` writes a dual-slot clipboard payload (`image/png` for external apps + `text/html` carrying a `data-whiteboard-v1` data attribute with the strokes + texts + shapes bundle). Pasting back inside the whiteboard reconstitutes the objects as live vectors; external paste lands as PNG. Selections containing images fall back to PNG-only (image-bytes round-trip deferred). DoS-capped at 5000 strokes / 5000 texts / 50000 samples per stroke. See [ADR 0017](decisions/0017-whiteboard-native-clipboard-format.md). |
| **`transform-many` composite op** | ✅ Complete | v1.3; multi-object move drags emit a single composite op carrying per-item before/after transforms (image / text rects + stroke dx/dy deltas) instead of N independent ops. One undo step reverses the whole group move; M3 sync prep — one transaction + one wire update per peer per gesture. |
...
| **Color eyedropper tool**         | 🟡 Descoped    | M2 — descoped at feel-test on tenet grounds; right-click + color picker (`Shift+C`) cover the use case. |
...
| **`StrokeStore` interface seam**  | ✅ Complete    | M2.1; abstracts stroke persistence behind `load/upsert/delete/clear/onRemoteChange` — the generic `ObjectStore<T>` contract shared by all four object stores (`objectstore.ts`). Local IDB is the only concrete implementation at v1; preserved as the integration point for a future Y.Doc-backed implementation. |
