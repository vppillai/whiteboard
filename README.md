...
**v1.5.0** — single-user, offline-first, production-ready for the indirect-input drawing case the project was built for. Lint, typecheck, 274 unit tests, and a Playwright e2e smoke suite run on every push (`bun test`, `bun run test:e2e`); latency budget (≤ 33 ms pen-to-photon) is feel-tested on Wacom Intuos at every milestone close.
