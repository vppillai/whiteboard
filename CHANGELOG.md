...
- **Mechanical extractions ahead of the Select-tool vtable refactor** (phase 1; pure moves with import updates, zero behavior change). The rotation-geometry quadruplication collapses into a new `geom.ts` — `ROTATION_EPSILON`, `rotateAroundPoint`, a generic rotated-rect AABB that `imageAABB` / `textAABB` / `shapeAABB` now delegate to, and the `ViewBBox` interface (previously declared in three render passes) — plus a shared `selectionoutline.ts` multi-selection outline painter (was three copies), and the SVG export now imports the arrow-head / fill-alpha constants from `rendershapes.ts` instead of re-deriving them. `tools/select.ts` sheds two self-contained blocks into sibling modules: the pure handle geometry (`tools/select/handles.ts` — handle positions, rotation handle, resize anchors, rotation-aware cursors, handle hit-tests) and the right-click contextual-menu DOM builders (`tools/select/menu.ts`, with the tool internals they closed over injected as explicit deps). `select.ts` drops from 2637 to 2187 lines; ADR 0014's migration trigger (4th object kind + kind-aware code growth) is what motivates the upcoming vtable step these extractions stage for.
- **The Select tool's per-kind dispatch is now an `ObjectBehavior<T>` vtable** (phase 2 — the step the extractions above staged for; zero observable behavior change: identical op kinds, payloads, and guard conditions for every gesture). [ADR 0014](docs/decisions/0014-select-tool-selection-union.md)'s migration trigger fired — shapes made a fourth object kind and the kind-aware code crossed the 300-LOC threshold — so the per-site `kind ===` dispatch (`selectSingleById` / `selectByIds` / `getView` / `startMultiDrag` / `tickMultiDrag` / `commitMultiDrag` / `commitDrag` / `deleteSelected`) and the four per-kind drag-commit helpers collapse into registry lookups against a new `tools/select/behaviors.ts`: `resolve` (live-object lookup, fails closed on stale ids), `viewParts` (ObjectView transform/rotation), `beginMultiDrag` (per-item closure handles, so each kind carries its own multi-drag state), `commitSingleDrag` (drag-end op emission), and `collectDeleteId` (the `delete-many` slot). Strokes keep their samples-translation model rather than being forced into a transform shape. One layer down, `ops.ts` gains `setTransformOn<T>` / `setRotationOn<T>` generics (mirroring `flipDeletedOn`), replacing the byte-identical per-kind transform / rotation setter triplets — the `Op` union and its serialized shapes are untouched. `select.ts` drops from 2187 to 1842 lines; the ADR gains an *Adopted* addendum recording the trigger.
- **The four object-store interfaces are unified behind one generic `ObjectStore<T>`.** The store seam exists so a future Y.Doc-backed sync implementation (ADR 0012) can plug in — but the four stores had drifted (`save`/`delete` on strokes, `update`/`hardDelete` on texts and shapes, `insert`/`updateMeta`/`hardDelete` on images), which would have meant four bespoke adapters instead of one. A new `objectstore.ts` defines the shared contract (`load` / `upsert` / `delete` / `clear` / `onRemoteChange`); stroke, text, and shape stores are now plain `ObjectStore<T>` aliases, and `ImageStore` extends it keeping its documented blob side channel (`insert(image, blob)` creation path, `loadBlob`, blob-aware `delete(id, blobRef)`, compaction-reporting `load`). Pure rename — persistence semantics, call ordering, and the `OpContext` callback surface (`saveStroke` et al.) are untouched.
