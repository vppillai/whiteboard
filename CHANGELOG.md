...
- **Color eyedropper tool.** New `I` key + right-click TOOL section pill ("Pick"). Modal tool conforming to ADR 0007. Samples color via `getImageData(1, 1)` on the strokes offscreen layer (not committed — grid pixels never sample, clean by construction); on pointerdown commits to active brush color and reverts to previous tool. Cursor visual: 16 px disc filled with the live-sampled color, with thin white-over-black contrast rings for theme robustness. Alpha < 0.5 holds the previous sample (no flicker over wipe-erased regions). Tool interface gains optional `onActivate(prevToolId, ctx)` so the eyedropper can capture its caller.
...
- **Eyedropper pill in toolpill cycle**: actually no — eyedropper is excluded from the toolpill cycle (`pen → eraser → lasso → pen`) because it's a modal "pick then revert" tool, not a sticky drawing/select mode. Reach it via `I` or right-click → TOOL → Pick.
